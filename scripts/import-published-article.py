...
rep = {'صالحیت':'صلاحیت','ایاالت':'ایالات','اختالف':'اختلاف','اطالعات':'اطلاعات',
       ...
       'برخالف':'برخلاف','مثالً':'مثلاً','معموالً':'معمولاً','عمالً':'عملاً',
       'کامالً':'کاملاً','اوالً':'اولاً','عالوة':'علاوه','عالوه':'علاوه',
       'محصوالت':'محصولات','تحوالت':'تحولات','مجموعهای':'مجموعه‌ای',
       'آییننامهها':'آیین‌نامه‌ها','اتحادیههای':'اتحادیه‌های','اصلاحیههای':'اصلاحیه‌های',
       'بخشهای':'بخش‌های','کارتلها':'کارتل‌ها','پروندهها':'پرونده‌ها',
       'پروندههایی':'پرونده‌هایی','آسیبهای':'آسیب‌های','نوآوریهایی':'نوآوری‌هایی',
       'سختترین':'سخت‌ترین','سختگیرانهتر':'سخت‌گیرانه‌تر','سختگیرانه':'سخت‌گیرانه',
       'بیانگیزه':'بی‌انگیزه','بهصرفه':'به‌صرفه','نیمهخصوصی':'نیمه‌خصوصی',
       'شبهدولتی':'شبه‌دولتی','همزمان':'هم‌زمان','روبهرو':'روبه‌رو',
       'برنامهریز':'برنامه‌ریز','برنامهریزی':'برنامه‌ریزی','نرخگذاری':'نرخ‌گذاری',
       'اندازهگیری':'اندازه‌گیری','اختصاصیافته':'اختصاص‌یافته',
       'تخصیصیافته':'تخصیص‌یافته','صورتجلسة':'صورت‌جلسه',
       'صورتجلسه':'صورت‌جلسه','صورتهای':'صورت‌های','صورتحساب':'صورت‌حساب',
       'مطرحشده':'مطرح‌شده','تنظیمشده':'تنظیم‌شده','اعمالشده':'اعمال‌شده',
       'اشارهشده':'اشاره‌شده','شدهاند':'شده‌اند','ساختهاند':'ساخته‌اند',
       'مواجهاند':'مواجه‌اند','عدهای':'عده‌ای','گونهای':'گونه‌ای',
       'رویهای':'رویه‌ای','محاسبهای':'محاسبه‌ای','شمارهای':'شماره‌ای',
       'میکنند':'می‌کنند','مینمایند':'می‌نمایند','میدارد':'می‌دارد',
       'میآورد':'می‌آورد','نقشآفرینی':'نقش‌آفرینی','مستنثی':'مستثنی',
       'پیشآمده':'پیش‌آمده','مهمترین':'مهم‌ترین','سهلگیرانه':'سهل‌گیرانه',
       'سادهتری':'ساده‌تری','تازه وارد':'تازه‌وارد','سرمایه گذاری':'سرمایه‌گذاری',
       'سرمایه‌گذاریها':'سرمایه‌گذاری‌ها','صرفنظر':'صرف‌نظر',
       'تعیینکننده':'تعیین‌کننده','تنظیم‌کنندهبخشی':'تنظیم‌کننده بخشی',
       'مقررات‌گذاریها':'مقررات‌گذاری‌ها','مقررات‌گذاریهای':'مقررات‌گذاری‌های',
       'قیمت‌گذاریها':'قیمت‌گذاری‌ها','تعرفهگذاری':'تعرفه‌گذاری',
       'نرخهای':'نرخ‌های','جدولها':'جدول‌ها','گرفتهاند':'گرفته‌اند',
       'هیچچیز':'هیچ‌چیز','پیشبینیپذیر':'پیش‌بینی‌پذیر',
       'بهماسبق':'به‌ماسبق','همانطور':'همان‌طور','کدامیک':'کدام‌یک'}
...
def clean(t):
    ...
    t=re.sub('\u202a([^\u202c]*)\u202c',ltr,t)
    t = re.sub(r'[\u200e\u200f\u202a-\u202e\u2066-\u2069]', '', t)
    t = re.sub(r'\s+', ' ', t).strip().replace('ي','ی').replace('ك','ک').replace('ة','ه')
...
for page_no,page in enumerate(raw.split('\f')):
    ...
    if not pl: continue
    if page_no:
        pl=[l for l in pl if l!='عم' and 'فصلنامه دانش حقوق' not in l and 'موازنه صلاحیت شورای رقابت' not in l and not re.fullmatch(r'\d{3}',l)]
    lines.extend(pl)
...
# Author-year citations remain available in the bibliography. Removing them
# from the reading text avoids disruptive LTR runs without deleting acronyms,
# explanatory parentheses, legal dates or the article's numbered notes.
body=re.sub(r'\s*\((?=[^()\n]*(?:\d{4}|[۱۲][۰-۹]{3}))(?=[^()\n]*:)[^()\n]*\)', '', body)
...
17:'«شماره‌گذاری»',19:'پرونده دیگری',20:'شکست‌های بازار'}
...
assert body.count('در پرونده اول')==1
body=body.replace('در پرونده اول','در پرونده اول [[17]]',1)
